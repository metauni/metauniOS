...
function Binder:Start()
	...
	for _, inst in pairs(CollectionService:GetTagged(self._tagName)) do
		-- print("add from gettagged", inst:GetFullName(), tostring(self))
		task.spawn(self._add, self, inst)
	end
	
	self._maid:AssignEach(CollectionService:GetInstanceAddedSignal(self._tagName):Connect(function(inst)
		-- print("add from added signal", inst:GetFullName(), tostring(self))
		self:_add(inst)
	end))
	self._maid:AssignEach(CollectionService:GetInstanceRemovedSignal(self._tagName):Connect(function(inst)
		self:_remove(inst)
	end))
end
...
function Binder:GetClassAddedSignal()
	...
	self._maid:AssignEach(self._classAddedSignal)
	return self._classAddedSignal
end
...
function Binder:GetClassRemovingSignal()
	...
	self._maid:AssignEach(self._classRemovingSignal)
...
function Binder:GetClassRemovedSignal()
	...
	self._maid:AssignEach(self._classRemovedSignal)
